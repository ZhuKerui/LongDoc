...
class FastCorefResolver:
    ...
    def __call__(self, doc: Doc, resolve_text=False, prefix='') -> Doc:
        """
        ...
        idx2nc = {nc.root.i: nc for nc in doc.ents if nc.label_ in ['PERSON']}
        new_clusters = []
            ...
            for cluster in clusters:
                ...
                if indices and doc.char_span(cluster[indices[0]][0], cluster[indices[0]][1]).root.i in idx2nc:
                    ...
                    for coref in cluster:
                        if coref != mention and not self._is_containing_other_spans(coref, all_spans) and coref[0] >= len(prefix):
